...
function addElement(element, type, scrollbar = true) {
...
function F(a) {
  ...
  function h(a) {
    ...
    document.removeEventListener(g ? "touchmove" : "mousemove", f, {
      passive: true,
    }),
      document.removeEventListener(g ? "touchend" : "mouseup", h, {
        passive: true,
      }),
  ...
  (w = u = b.clientX),
    (x = v = b.clientY),
    (y = !1),
    (A = H({ element: a.target, type: "Y" }, w, x, a.timeStamp, a));
  if (!A.length) return !0;
  let d = a.target,
    e = setTimeout(function () {
      (e = 0), (c = L(d));
    }, 50);
  document.addEventListener(g ? "touchmove" : "mousemove", f, {
    passive: true,
  }),
    document.addEventListener(g ? "touchend" : "mouseup", h, { passive: true });
}
...
function G(a, b, c, d, scrollbar) {
  ...
  function ba() {
    ...
    e.cleanup && e.cleanup(),
      ...
      e.addEventListener("webkitAnimationEnd", e.normalEnd, { passive: true }),
      R(e, b.name, b.time),
      N.forEach(function (a) {
  ...
  function bd(b) {
    (e.style.webkitTransform = a.update(b + P)),
      (e.scrollableOffset = P),
      N.forEach(function (a) {
  ...
  function be(b) {
    u &&
      ...
      e.removeEventListener("webkitAnimationEnd", e.normalEnd, {
        passive: true,
      }),
      delete e.earlyEnd,
      delete e.normalEnd;
  ...
  return (
    u &&
      (X(u, _),
      scrollbar.render
        ? u.parentNode || e.parentNode.appendChild(u)
        : u.parentNode && e.parentNode.removeChild(u)),
    e.earlyEnd &&
      (R(e),
      N.forEach(function (a) {
        R(a.node);
...
function H(a, b, c, d, f) {
  ...
  for (let g = 0; g < ELEMENT.length; ++g) {
    if (
      [f.target, f.currentTarget, f.srcElement].some((e) =>
        ELEMENT[g].element.contains(e)
      )
    )
      I(ELEMENT[g], e, b, c, d);
  }
...
function J({ element, type }, b, c, d) {
  if (ELEMENT.some((item) => item.element === element)) {
    ...
    if (D[type]) {
      ...
      return (
        (h.direction = type),
        (h.paginated = element.className.indexOf("paginated") != -1),
        h
      );
    }
  }
...
function L(a) {
  let b = [];
  for (let c = a; c; c = c.parentNode)
    c.nodeType == 1 &&
      ((c.className = (c.className ? c.className + " " : "") + "touched"),
      b.push(c));
  return b;
}
...
function onload() {
  document.addEventListener(g ? "touchstart" : "mousedown", F, {
    passive: true,
  });
  E();
}
...
const _VueTouchScroll = {
  install(Vue) {
    Vue.directive("touch-scroll", this.directive);
    Vue.component("VueTouchScroll", this.VueTouchScroll);
  },
  directive: {
    bind(element, { arg, value }) {
      addElement(element, arg, value);
    },
...
export default _VueTouchScroll;

export const VueTouchScroll = _VueTouchScroll.VueTouchScroll;
